...
def read_prc_csv(tic):
    ...
    # <COMPLETE THIS PART>
    tic_name=tic.lower()
    file_name=f"{tic_name}_prc.csv"
    csv=os.path.join(cfg.DATADIR, file_name)
    df_raw=pd.read_csv(csv, parse_dates=['Dates'], index_col='Date')
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    for i in tickers:
        df_i = read_prc_csv(i)
        df_iw = df_i.loc[:, [prc_col]]
        df_iw.columns = [f"{i.lower()}"]
        df_raw = df_raw.join(df_iw, how='outer')
...
def mk_ret_df(prc_df):
    ...
    df_mktw= pd.read_csv(cfg.FF_CSV, parse_dates=['Date'], index_col='Date')

    df_mktr= df_mktw.loc[:, ['mkt']]
    df_mktw.sort_index(inplace=True)

    prc_df.sort_index(inplace=True)

    prc_r = prc_df.pct_change()

    df= prc_r.join(df_mktr, how='left')
...
def mk_aret_df(ret_df):
    ...
    """
    # <COMPLETE THIS PART>
    col=ret_df.iloc[:,:-1].columns

    for i in col:
        ret_df[i]= ret_df[i] - ret_df['mkt']
...
def get_avg(df, col, year):
    ...
    selected = df.loc[str(year), col].dropna()
    avg = selected.mean()
    return avg
...
def get_ew_rets(df, tickers):
    ...
    """
    #<COMPLETE THIS PART>
    data = df[tickers].dropna().mean(axis=1)
    data.index = pd.to_datetime(data.index)
    series = pd.Series(data=data, index=data.index)
    return series
...
def get_ann_ret(ser, start, end):
    ...
    # <COMPLETE THIS PART>
    ser = ser.dropna()
    tot_ret = ser.loc[start:end].add(1).prod()
    N = len(ser.loc[start:end])
    scalar = tot_ret ** (252 / N) - 1
    return scalar
